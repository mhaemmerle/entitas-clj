...
(defn entry-creation-bench [repository num-entities ctype1 ctype2]
  (reduce (fn [acc i]
            (let [[new-acc entity] (r/add-entity acc (e/new :foo))]
              (condp = (mod i 25)
                0 (first (cr/add-component new-acc entity {:type ctype1}))
                1 (first (cr/add-component new-acc entity {:type ctype2}))
                new-acc)))
...
(defn exchange-component-in-all-entities [repository entities]
  (let [[i result] (reduce (fn [[idx acc] entity]
                             (let [[r _] (cr/exchange-component acc entity {:type :foo})]
...
(defn run-test [entity-count]
  (let [ctype1 :foo
        ctype2 :bar
        r1 (with-time (entry-creation-bench (r/new) entity-count ctype1 ctype2))
